...
def create_api_spec_plugin(app):
    api_spec_plugin = ApiSpecPlugin(
        app=app,
        # Объявление списка тегов с их описанием,
        # поэтому API организован в группы. это необязательно.
        tags={
            "Tag": "Tag API",
            "User": "User API",
            "Author": "Author API",
...
def init_api(app):
    event_plugin = EventPlugin()  # Подключение EventPlugin
    api_spec_plugin = create_api_spec_plugin(app)
    permission_plugin = PermissionPlugin(strict=False)  # инициализируем PermissionPlugin
    api = Api(
        app,
        plugins=[
            event_plugin,
